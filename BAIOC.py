...
def send_telegram_message(file_path, token, chat_id)
    url = fhttpsapi.telegram.orgbot{token}sendDocument
...
def fetch_ohlcv_data(exchange, symbol, timeframe, limit=None)
    try
        ...
        print(fVeri alınırken hata oluştu ({symbol} - {timeframe}) {e})
        return None

# RMA (Eksponansiyel Hareketli Ortalama) fonksiyonu
def rma(series, length)
    return series.ewm(alpha=1length, adjust=False).mean()
...
# Pairs analiz fonksiyonu (Belirtilen koşullara göre filtreleme)
    ...
    for timeframe in timeframes
        ...
        if data is not None and not data.empty
            ...
            if data['CHOP'].iloc[-1] = 61.78
                ...
                chop_condition = data['CHOP'].iloc[-1] = 61.78
                adx_condition = data['ADX'].iloc[-1] = 25
                dmi_condition = data['Positif_DI'].iloc[-1]  data['Negatif_DI'].iloc[-1]
                rsi_condition = data['RSI'].iloc[-1] = 50

                if chop_condition and adx_condition and dmi_condition and rsi_condition
                    ...
                    pair_data[f'Positif_Ustunde_Negatif_{timeframe}'] = data['Positif_Ustunde_Negatif'].iloc[-1] if not data['Positif_Ustunde_Negatif'].isnull().iloc[-1] else NA
            ...
            print(fUyarı {symbol} sembolü için {timeframe} zaman diliminde veri bulunamadı.)
            continue
    return pair_data

# Kombine analiz sonuçlarını txt dosyasına kaydetme
def save_results_to_txt(pairs, filename='Binance All In One Conditions.txt')
    with open(filename, 'w') as f
        f.write(Binance Farklı Zaman Dilimlerinde Çıkan Semboller (Binance All In One Conditions)nn)
        ...
        for pair in multi_timeframe_pairs
            ...
            for timeframe in pair['Timeframes']
                f.write(f{timeframe} Chop {pair.get(f'CHOP_{timeframe}', 'NA')}n)

            f.write(-  40 + n)

            for timeframe in pair['Timeframes']
                f.write(f{timeframe} ADX {pair.get(f'ADX_{timeframe}', 'NA')}n)

            f.write(-  40 + n)

            for timeframe in pair['Timeframes']
                f.write(f{timeframe} Pozitif DMI {pair.get(f'Positif_Ustunde_Negatif_{timeframe}', 'NA')}n)

            f.write(-  40 + n)

            for timeframe in pair['Timeframes']
                f.write(f{timeframe} RSI {pair.get(f'RSI_{timeframe}', 'NA')}n)
...
txt_filename = 'Binance All In One Conditions.txt'
save_results_to_txt(analyzed_pairs, filename=txt_filename)

# Telegram botuyla txt dosyasını gönderme
API_TOKEN = '5141924896AAGUMzfDSu1N9EXFQBiy7FBJTYah9Ej-9I8'
