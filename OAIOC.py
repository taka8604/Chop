...
# OKX borsasından OHLCV verilerini almak için fonksiyon
...
# Pairs analiz fonksiyonu (CHOP değeri 61.70'ten büyükse, ADX 25 ve üzeri, Pozitif DMI Negatif DMI üstündeyse ve RSI 50 ve üzeriyse dahil et)
def analyze_pair(exchange, symbol, timeframes):
    ...
    for timeframe in timeframes:
        ...
        if data is not None and not data.empty:
            ...
            dmi_data = directional_movement_index(data, dilen=14, adxlen=14)  # DMI hesaplama
            ...
            # Burada şartları ekliyoruz:
            if (data['CHOP'].iloc[-1] > 61.70 and 
                data['ADX'].iloc[-1] >= 25 and 
                data['Positif_DI'].iloc[-1] > data['Negatif_DI'].iloc[-1] and 
                data['RSI'].iloc[-1] >= 50):
...
def save_results_to_txt(pairs, filename='okx_analysis_results.txt'):
    with open(filename, 'w') as f:
        f.write("OKX Farklı Zaman Dilimlerinde Çıkan Semboller:\n\n")
...
# OKX Exchange bağlantısı oluşturma
exchange = ccxt.okx()
...
# Zaman dilimleri
timeframes = ['1h', '2h', '4h', '6h', '12h', '1d', '1w', '1M']
...
txt_filename = 'okx_analysis_results.txt'
save_results_to_txt(analyzed_pairs, filename=txt_filename)

# Telegram botuyla txt dosyasını gönderme
API_TOKEN = '5141924896:AAGUMzfDSu1N9EXFQBiy7FBJTYah9Ej-9I8'  # Telegram API token'ınızı buraya yazın
CHAT_ID = '@signalakin2023'  # Telegram kanalınızın ID'sini yazın
