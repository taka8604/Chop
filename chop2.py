...
def choppiness_index(data, length=14):
    ...
    # Sıfır farkları kontrol et
    price_range = highest_high - lowest_low
    price_range = price_range.replace(0, np.nan)  # Sıfırları NaN ile değiştir
    print(f"Price range after NaN replacement: {price_range.tail(10)}")  # Hata ayıklama

    # NaN değerleri 1 ile değiştirerek hesaplamaya devam et
    ci = 100 * np.log10(atr_sum / price_range.fillna(1)) / np.log10(length)
...
pairs_Ustunde_61_8_1h = [pair for pair in analyzed_pairs if pair.get('CHOP
